...
export default function WellboreA4Print({ well: wellProp, onClose, hideSchematic }: WellboreA4PrintProps) {
  ...
  const resolvedLabels = (() => {
    ...
    if (prodCsg && parseSizeToNumber(prodCsg.casingSize) !== 7) {
      ...
      csgLabelList.push({
        ...
        renderText: (y: number) => (
          <g key="blueprint-prod-csg-text">
            <text x={239} y={y - 2} textAnchor="start" fontSize="11.5" fontWeight="bold">CSG: {formatCasingSize(prodCsg.casingSize)} {prodCsg.grade || 'J55'}</text>
    ...
    // Dynamic braced CSG labels to avoid any collisions
    const bracedCsgLabelList: ResolutionLabel[] = [];
    sortedCasings
      .filter(c => parseSizeToNumber(c.casingSize) > 2.5 && parseSizeToNumber(c.casingSize) !== 7)
      .forEach((csg, index) => {
        const csgR = parseSizeToNumber(csg.casingSize) * 4.5;
        const preferredY = 100 + (index * 135);
        ...
        bracedCsgLabelList.push({
          ...
          renderText: (y: number) => (
            <g key={`csg-label-braced-${index}`}>
              <text x={224} y={y + 4} fontSize="13" fontWeight="bold" className="font-sans">CSG</text>
              <text x={250} y={y + 14} fontSize="34" fontWeight="light" fill="#000">{"{"}</text>
              <g transform={`translate(270, ${y - 18})`} fontSize="13" fontWeight="bold" className="font-sans" fill="#000">
    ...
    // Dynamic braced TBG label to avoid collisions (with fallback to 7" casing values if empty)
      ...
      tbgLabelList.push({
        ...
        renderText: (y: number) => (
          <g key="blueprint-tbg-brace-text">
            <text x={224} y={y + 4} fontSize="13" fontWeight="bold" className="font-sans">TBG</text>
            <text x={250} y={y + 14} fontSize="34" fontWeight="light" fill="#000">{"{"}</text>
            <g transform={`translate(270, ${y - 18})`} fontSize="13" fontWeight="bold" className="font-sans" fill="#000">
              <text x={0} y={11}>Ø : {displayOd || ''}</text>
              <text x={0} y={25}>Gr. : {displayGrade || ''}</text>
              <text x={0} y={39}>Lbs. : {displayWeight || ''}</text>
            </g>
          </g>
        )
      });
    ...
    const toolLabels: ResolutionLabel[] = computedTools
      ...
      const isSpecialLabelStyle = isPacker || isSeatingNipple || isShoe || isMandrin;
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/90 z-50 overflow-y-auto p-4 md:p-8 flex flex-col items-center print:p-0 print:bg-white select-none" id="a4_print_wrapper">
      ...
      {/* Floater Header bar */}
      <div 
        className="w-[820px] bg-white/95 backdrop-blur-md border border-slate-200/80 p-4 rounded-2xl mb-6 flex flex-col print:hidden relative z-10" 
        id="print_controls_bar"
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3.5">
            <div className="bg-gradient-to-b from-slate-100 to-slate-50 border border-slate-200 text-slate-600 p-2.5 rounded-xl shadow-sm">
              <FileText className="w-5 h-5" />
            </div>
            <div className="flex flex-col">
              <h2 className="text-[15px] font-extrabold text-slate-800 font-sans tracking-tight">Print & Export Viewer</h2>
              <p className="text-[12px] text-slate-500 font-medium mt-0.5">A4 Professional Technical Card</p>
            </div>
          </div>

          <div className="flex items-center gap-4">
            {/* Scale Toggle inside Header Bar */}
            <div className="flex items-center bg-slate-100/80 p-1 rounded-xl border border-slate-200/80 shadow-inner">
              <div className="px-3 flex items-center gap-1.5 text-slate-400 border-r border-slate-200/80 mr-1">
                <span className="text-[11px] uppercase tracking-wider font-bold">Échelle</span>
              </div>
              <button
        ...
        {isEmbedded && (
          <div className="bg-amber-50 border border-amber-200/60 rounded-xl p-3 mt-3 flex items-start gap-3 shadow-sm">
            <div className="bg-amber-100 p-1.5 rounded-lg shrink-0 mt-0.5 text-amber-600">
              <ExternalLink className="w-4 h-4" />
            </div>
            <div className="flex flex-col gap-0.5">
              <span className="text-sm font-bold text-amber-900">Browser Security Restriction</span>
              <p className="text-xs text-amber-800 leading-relaxed font-medium">
                Browsers block the Print dialog inside preview frames. Click <strong>"Open in New Tab"</strong> above to launch full-screen, then use the Print button.
              </p>
            </div>
          </div>
        )}
      ...
      >
        ...
        <div className="w-full h-full flex flex-col p-2 relative z-10">
                  ...
                  {resolvedLabels.map((label) => {
                    ...
                    return (
                      <g key={`resolved-label-${label.id}`}>
                        {hasSlant ? (
                          <path
                            d={`M ${label.startX} ${label.targetY} H ${elbowX} L ${elbowX + 6} ${label.y} H ${lineXEnd}`}
                            fill="none"
                            stroke="#000"
                            strokeWidth="0.8"
          ...
          <div className="w-full mt-2 pt-2 border-t border-slate-300 flex items-center justify-end text-[9.5px] text-slate-500 shrink-0 font-mono" id="a4_page_footer">
